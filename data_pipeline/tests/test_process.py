...
def test_process_data_temporal_split():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(train_df) == 3, (
            "Expected 3 training records "
            f"(year <= 2010), got {len(train_df)}"
        )
        assert len(prod_df) == 2, (
            "Expected 2 production records "
            f"(year > 2010), got {len(prod_df)}"
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for feature in expected_audio_features:
            assert feature in train_df.columns, (
                f"Missing audio feature '{feature}' "
                "in training data"
            )
...
def test_process_data_year_boundary_condition():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(train_df) == 2, (
            "Expected 2 training records "
            f"(2009, 2010), got {len(train_df)}"
        )
        assert len(prod_df) == 2, (
            "Expected 2 production records "
            f"(2011, 2012), got {len(prod_df)}"
        )

        # Verify exact years in each split
        assert set(train_df["year"]) == {2009, 2010}, (
            "Expected years {2009, 2010} in train, "
            f"got {set(train_df['year'])}"
        )
        assert set(prod_df["year"]) == {2011, 2012}, (
            "Expected years {2011, 2012} in prod, "
            f"got {set(prod_df['year'])}"
        )
