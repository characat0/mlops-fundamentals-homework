...
def test_process_data_temporal_split():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            len(train_df) == 3
        ), f"Expected 3 training records (year <= 2010), got {len(train_df)}"
        assert (
            len(prod_df) == 2
        ), f"Expected 2 production records (year > 2010), got {len(prod_df)}"
...
def test_process_data_year_boundary_condition():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            len(train_df) == 2
        ), f"Expected 2 training records (2009, 2010), got {len(train_df)}"
        assert (
            len(prod_df) == 2
        ), f"Expected 2 production records (2011, 2012), got {len(prod_df)}"
        # Verify exact years in each split
        assert set(train_df["year"]) == {
            2009,
            2010,
        }, f"Expected years {{2009, 2010}} in train, got {set(train_df['year'])}"
        assert set(prod_df["year"]) == {
            2011,
            2012,
        }, f"Expected years {{2011, 2012}} in prod, got {set(prod_df['year'])}"
